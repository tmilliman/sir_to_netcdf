...
cp -p ../geotiffs/${region}/*/${region}-msfa-mean-db-*-[JOA]*.tif ./
cp -p ../geotiffs/${region}/*/${region}-msfa-std-db-*-[JOA]*.tif ./

if [ "${region}" != "Ber" ]
then
    # reproject and rename files
    for file in `ls ${region}-msfa-mean-db-*-[JOA]*.tif`
    do
        echo ${file}
    ...
    for file in `ls ${region}-msfa-std-db-*-[JOA]*.tif`
    do
        echo ${file}
        target=`echo ${file} | sed 's/-/_/g'`
        gdalwarp -t_srs epsg:4326 -tr 0.05 0.05 -te ${warp_extent} -overwrite -tap -r average ${file} ${target}
