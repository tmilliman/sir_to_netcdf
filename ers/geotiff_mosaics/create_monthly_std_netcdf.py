...
def time_index_from_filenames(filenames):
    return pd.DatetimeIndex(
        [pd.Timestamp(f[16:20] + f[21:23] + "15") for f in filenames]
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create ERS1/2 monthly std netcdf file"
    )

    args = parser.parse_args()

    # get list of files
    filepattern = "ers_ers[12]_std_db_*_[01][0-9].tif"
    ...
    ds.attrs["title"] = "ERS1/2 sigma-0 Monthly Std. Dev."
    ds.attrs["description"] = "ERS1/2 std from monthly mean ers1/2 sigma-0"
    ...
    outfile = "ERS_monthly_land_sig0_StdDev.nc"
