...
cp -p ../geotiffs/${region}/*/${region}-quev-monthly-mean-db-*.tif ./
cp -p ../geotiffs/${region}/*/${region}-quev-monthly-std-db-*.tif ./
...
then
    ...
    for file in `ls ${region}-quev-monthly-mean-db-*.tif`
    do
        echo ${file}
        target=`echo ${file} | sed 's/quev-monthly/qscat-quev/' | sed 's/-/_/g'`
        gdalwarp -t_srs epsg:4326 -tr 0.05 0.05 -te ${warp_extent} -overwrite -tap -r average ${file} ${target}
        rm ${file}
    done
    
    # repeat for std images
    for file in `ls ${region}-quev-monthly-std-db-*.tif`
    do
        echo ${file}
        target=`echo ${file} | sed 's/quev-monthly/qscat-quev/' | sed 's/-/_/g'`
        gdalwarp -t_srs epsg:4326 -tr 0.05 0.05 -te ${warp_extent} -overwrite -tap -r average ${file} ${target}
    ...
    do
        echo ${file}
        target=`echo ${file} | sed 's/quev-monthly/qscat-quev/' | sed 's/-/_/g' | sed 's/\.tif/\_1\.tif/'`
